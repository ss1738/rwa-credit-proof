...
Rust, 5 tests green on Mini, demo shows solvent to ALLOW, insolvent to BLOCK, tampered to BLOCK.

## Architecture
- **Fold (the edge):** each loan = one Nova IVC step folding a running `(collateral, liability, kyc_AND)`
  accumulator. N loans yields O(1) verifier work + one proof.
- **Compress to on-chain:** Nova → Spartan → final Groth16 (BN254), verified on Solana via `alt_bn128`
  pairing syscalls.
...
1. **Prover scales:** 10k-loan proof in < ~30 min on the Minis. Hours means the model doesn't scale.
...
4. **Demand real:** at least 1 private-credit protocol signs an LOI / written "we'd pilot this". Zero
   after 10 convos means shelve.

## Kill criteria
Prover can't reach 10k in reasonable time and folding doesn't fix it; or on-chain verify can't fit and
the fallback kills the value prop; or zero design-partner interest. Any of these means shelve.
