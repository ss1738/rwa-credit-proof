...
## Tier 3: infrastructure leverage (one integration reaches many issuers)
...
5. **Credibility warm-up (optional):** open-source the Solana `alt_bn128` verifier as a standalone
   dev tool, for visibility in the RWA/Solana dev community, generating inbound instead of outbound.

## Sequence
Start with **Credix + Huma + Centrifuge** (3 parallel, low-friction, high-pain). Land one, then use it
as the named reference to open **Maple / Apollo**. Do NOT start at the top of the market solo.
